...
for idx in range(len(epocs['test'])):
    ...
    pyplot.scatter(range(counter, train_len), train, s=5, c="b")
    pyplot.scatter(range(train_len, test_len), test, s=5, c="r")
    pyplot.vlines(test_len + 1, min(train + test), max(train + test))
    pyplot.hlines(np.mean(train), counter, test_len, colors="b", linestyles="dashed")
